...
startInstr = GetLabelAddress("DATABUSLOOP1",  buildSrc + "/bl2u/bl2u.dump") - 12
populateInstr = GetLabelAddress("DATABUSLOOP1",  buildSrc + "/bl2u/bl2u.dump") + 4
didntFailInstr = GetLabelAddress("DATABUSLOOP1",  buildSrc + "/bl2u/bl2u.dump") + 18
endInstr = GetLabelAddress("ENDDATABUSTEST",  buildSrc + "/bl2u/bl2u.dump")
memoryReg = "r1"
statusFlagReg = "r3"
...
def PopulatingDoneEvent():
    if random.random() < 0.05: #Much lower probability of failing - runs very fast so this is fine
        readValue = debugger.readRegister("r5")
        
        bitPos = random.randint(0, 31)
        writeValue = readValue ^ (1 << bitPos)
        
        debugger.writeRegister("r5", writeValue) # Write negated value to register holding result to induce error
        print("Putting incorrect value of " + hex(writeValue & 0xffffffff) + " at address to related register")
