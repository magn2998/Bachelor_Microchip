...
startInstr = GetLabelAddress("RedoWriteBurstLoop",  buildSrc + "/bl2u/bl2u.dump") - 12
populateInstr = GetLabelAddress("WRITEBURSTLOOP2",  buildSrc + "/bl2u/bl2u.dump") - 4
endInstr = GetLabelAddress("ENDBURSTTEST",  buildSrc + "/bl2u/bl2u.dump")
memoryReg = "r4"
statusFlagReg = "r4"
...
def EndOfTestEvent(expectedAddr, shouldFail):
    ...
    failedMemAddress = debugger.readRegister("r0") # Changes since it is actually two Assemble code sections...
...
def PopulatingDoneEvent():
    if random.random() < 0.75:
        addr = random.randint(0x60000000, 0x60007fff)
        readValue = debugger.readMemoryValue(addr, 8)
        
        bitPos = random.randint(0, 7)
        writeValue = readValue ^ (1 << bitPos)
        
        debugger.writeMemoryValue(addr, writeValue, 8) # Write negated value to induce error
        print("Putting incorrect value of " + hex(writeValue & 0xff) + " at address " + hex(addr))
