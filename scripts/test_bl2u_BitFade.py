...
startInstr = GetLabelAddress("FADELOOP1",  buildSrc + "/bl2u/bl2u.dump") - 8
populateInstr = GetLabelAddress("FADELOOP2",  buildSrc + "/bl2u/bl2u.dump") - 4
endInstr = GetLabelAddress("ENDFADETEST",  buildSrc + "/bl2u/bl2u.dump")
memoryReg = "r3"
statusFlagReg = "r4"
...
def EndOfTestEvent(expectedAddr, shouldFail):
    ...
    failedMemAddress = debugger.readRegister("r0") - 4 # Changes since it is actually two Assemble code sections...
...
while(True):
    ...
    if hitBreakId == EndOfPopulation:
        ...
        debugger.writeRegister("r0", 0x60000000) # Reset Memory Address of second loop.. 
